...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        下面这段代码主要
        1、去除了重复的病例
        2、将"survival_months"这一列通过四分位数和censorship分成了8类，然后作为"label"插入到了slide_data，patient_data，patients_df
        """
        ...
        # 将label_col按分位数分为n_bins类
        disc_labels, q_bins = pd.qcut(uncensored_df[label_col], q=n_bins, retbins=True, labels=False)
        q_bins[-1] = slide_data[label_col].max() + eps  # q_bins为4分位的分隔值
        ...
        # 上述for循环生成病例与图片对应的字典patient_dict

        slide_data = patients_df  # 此时的slide_data 去除了重复病例，插入了标签label列
        slide_data.reset_index(drop=True, inplace=True)  # 重新按顺序生成索引
        slide_data = slide_data.assign(slide_id=slide_data['case_id'])  # 插入新的列slide_id且其值为slide_data['case_id']
        ...
        for i in slide_data.index:
            key = slide_data.loc[i, 'label'] #通过i来索引行，‘label’来指定列
            slide_data.at[i, 'disc_label'] = key   # 在指定的行列赋值
            censorship = slide_data.loc[i, 'censorship']
            key = (key, int(censorship))
            slide_data.at[i, 'label'] = label_dict[key]  # 进一步按censorship为0，1把label分成了八类
        #
        self.bins = q_bins
        ...
        slide_data = slide_data[new_cols]  # 将最后两列移到最前面来
        self.slide_data = slide_data
        self.metadata = slide_data.columns[:12] # 保存前12列数据
        self.modal = modal
        self.cls_ids_prep()
    ...
    def cls_ids_prep(self):
        ...
        for i in range(self.num_classes):
            self.patient_cls_ids[i] = np.where(self.patient_data['label'] == i)[0]  # self.patient_cls_ids记录各类别标签的索引
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str = 'train', scaler=None):
        split = all_splits[split_key] # 获取train病例ID
        split = split.dropna().reset_index(drop=True)  #去除掉列中nan的数据

        if len(split) > 0:
            mask = self.slide_data['slide_id'].isin(split.tolist()) # 将slide_data['slide_id']中属于split.tolist()的病例设置为True
    ...
    def return_splits(self, from_id: bool = True, csv_path: str = None):
        if from_id:
            ...
            scalers = train_split.get_scaler()  # 对genomic_features创建标准化器
            train_split.apply_scaler(scalers=scalers) # 对训练集genomic_features创建标准化器
            val_split.apply_scaler(scalers=scalers)   # 对测试集genomic_features创建标准化器
...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    ...
    def __getitem__(self, idx):
        case_id = self.slide_data['case_id'][idx]   # 获取病例ID号
        label = self.slide_data['disc_label'][idx]  # 4分为分出来的4类标签
        event_time = self.slide_data[self.label_col][idx] # 病人生存时间
        c = self.slide_data['censorship'][idx] # censorship为0，1
        slide_ids = self.patient_dict[case_id] # 获取ID号对应的.svs文件
                ...
                if self.modal == 'path':
                    ...
                    for slide_id in slide_ids:
                        ...
                        try:
                            ...
                            print('FileNotFound: ', wsi_path)
                            wsi_bag = torch.randn(8, 1024)
                            path_features.append(wsi_bag)
                            continue
                    path_features = torch.cat(path_features, dim=0) # 一张WSI肿瘤区域的特征
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, modal, signatures=None, data_dir=None, label_col=None, patient_dict=None,
        ...
        for i in range(self.num_classes):
            self.slide_cls_ids[i] = np.where(self.slide_data['label'] == i)[0]  # 分别获得slide_data中对应类别的索引

        # --> Initializing genomic features in Generic Split
        self.genomic_features = self.slide_data.drop(self.metadata, axis=1) # 去除掉不属于基因组图谱的信息，剩下的就是基因组学特征
        ...
        if self.signatures is not None:
            ...
            for col in self.signatures.columns:
                omic = self.signatures[col].dropna().unique()   # 获取对应标志物的类别
                omic = np.concatenate([omic + modal for modal in ['_mut', '_cnv', '_rnaseq']])
                omic = sorted(series_intersection(omic, self.genomic_features.columns)) # 找到3个模态的标志物类别与genomic_features类别的交集
